...
# CloudWatch Insights コンソールで「Saved queries」から実行できる
resource "aws_cloudwatch_query_definition" "error_logs" {
  name = "${var.project}/error-logs"

  log_group_names = ["/ecs/${var.project}/api"]
...
# 閾値: cpu_alarm_threshold % (デフォルト 1%) — 学習環境で発火させやすくするため低めに設定
resource "aws_cloudwatch_metric_alarm" "ecs_cpu" {
  ...
  alarm_description   = "ECS CPU 使用率が ${var.cpu_alarm_threshold}% 以上になりました"
