...
resource "aws_iam_role" "github_actions_deploy" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringLike = {
          "token.actions.githubusercontent.com:sub" = "repo:takao-Tokunaga/aws-study:*"
        }
        StringEquals = {
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          ...
          "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.project}-frontend-task-role"
        ]
      },
      {
        Effect   = "Allow"
